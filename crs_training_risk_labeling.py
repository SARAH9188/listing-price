...
class CRSValidationRiskLabeling:
    def __init__(self, spark, threshold_df=None, static_rule_df=None, dynamic_rule_df=None,
        ...
        print("CRSPredictionRiskLabeling")
        self.spark = spark
        self.threshold_df = threshold_df
        self.tdss_dyn_prop_initial = tdss_dyn_prop
        ...
        if len(dynamic_rule_df.columns) > 0:
            ...
            self.dynamic_rule_df = crs_empty_data_frame_instances.get_emtpy_data_frame(
                CRS_Default.OVERALL_RISK_SCORE_REGULATORY)
        ...
        if len(static_rule_df.columns) > 0:
            ...
            self.static_rule_df = crs_empty_data_frame_instances.get_emtpy_data_frame(
                CRS_Default.OVERALL_RISK_SCORE_STATIC)
        ...
        self.un_supervise_df = self.un_supervise_df.withColumn(CRS_Default.PARTY_KEY_COL,
        ...
        try:
            self.crsUseManualThrdsForValidationFlag = JsonParser().parse_dyn_properties(tdss_dyn_prop, "UDF_CATEGORY",
                                                                CRS_Default.crsUseManualThrdsForValidation, "str")

        except:
            self.crsUseManualThrdsForValidationFlag = "False"

        if (self.crsUseManualThrdsForValidationFlag is None):
            self.tdss_dyn_prop = None
        else:
            if (self.crsUseManualThrdsForValidationFlag.lower() == "true"):
                self.tdss_dyn_prop = tdss_dyn_prop
            else:
                self.tdss_dyn_prop = None
        print("self.crsUseManualThrdsForValidationFlag", self.crsUseManualThrdsForValidationFlag)
        try:
            self.indi_coprp = JsonParser().parse_dyn_properties(tdss_dyn_prop, "UDF_CATEGORY",
                                                                CRS_Default.crsIndAndCorIdentifiers, "json")
        except:
    ...
    def risk_label_using_threshold(self):
        ...
        if supervise_num_record > 0:
            ...
            try:
                ...
                supervised_threshold_df = self.threshold_df \
                    .filter(F.col(CRS_THRESHOLDS.threshold_type) == CRS_Default.THRESHOLD_TYPE_Supervise).filter(
                    F.col(CRS_THRESHOLDS.model_id) == supervised_model_id)
            ...
            try:
                ...
                supervised_threshold_df_corp = self.threshold_df \
                    .filter(F.col(
                    CRS_THRESHOLDS.threshold_type) == CRS_Default.THRESHOLD_TYPE_Supervise + CRS_Default.crsCorporate).filter(
                    F.col(CRS_THRESHOLDS.model_id) == supervised_model_id)
                print("supervised_model_id***** from supervised df", supervised_model_id)
            except:
            ...
            try:
                ...
                supervised_threshold_df_indi = self.threshold_df \
                    .filter(F.col(
                    CRS_THRESHOLDS.threshold_type) == CRS_Default.THRESHOLD_TYPE_Supervise + CRS_Default.crsIndividual).filter(
                    F.col(CRS_THRESHOLDS.model_id) == supervised_model_id)
                print("supervised_model_id***** from supervised df", supervised_model_id)
            except:
            ...
            if (supervised_threshold_df.head(1) != 0):
                try:
                    ...
                    print("supervise_high_risk_threshold***** from supervised df", supervise_high_risk_threshold,
            ...
            if (supervised_threshold_df_corp.head(1) != 0):
                try:
                    ...
                    print("supervise_high_risk_threshold***** from supervised df", supervise_high_risk_threshold_corp,
                          supervise_low_risk_threshold_corp)
                except:
            ...
            if (supervised_threshold_df_indi.head(1) != 0):
                try:
                    ...
                    print("supervised_threshold_df_indi***** from supervised df", supervise_high_risk_threshold_indi,
                          supervise_low_risk_threshold_indi)
                except:
            ...
            def get_threshold(manual_spec_threshold, df_spec_threshold, manual_com_threshold, df_com_threshold,
                              type=""):
            ...
            print("Supervised thresholds extracted from df are", supervise_high_risk_threshold,
                  supervise_low_risk_threshold)
            ...
            if (CUSTOMERS.individual_corporate_type in self.supervise_df.columns):
                ...
                if (final_corp_high_threshold is not None and final_corp_low_threshold is not None) & (
                    ...
                    self.supervise_df_corp = CRSValidationRiskLabeling \
                    ...
                    self.supervise_df_indi = CRSValidationRiskLabeling \
                        .get_risk_level(individual_df, CRS_Default.Prediction_Prob_1_xgb,
                                        final_indi_high_threshold, final_indi_low_threshold,
                                        self.expl_cate[CRS_Default.THRESHOLD_TYPE_Supervise], self.tt_version)

                    self.supervise_df = self.supervise_df_corp.union(
                        self.supervise_df_indi.select(*self.supervise_df_indi.columns))
            else:
                print("CUSTOMERS.individual_corporate_type not present so normal the labelling technique")
                if (supervise_high_risk_threshold is not None or thresholds_high is not None) & (
                        supervise_low_risk_threshold is not None or thresholds_low is not None):
                    ...
                    self.supervise_df = CRSValidationRiskLabeling \
            ...
            self.supervise_df = self.supervise_df.withColumn(CRS_Default.RISK_LEVEL_COL,
        ...
        if un_supervise_num_record > 0:
            ...
            print("Unsupervised thresholds extracted are", un_supervise_high_risk_threshold,
            ...
            if (un_supervise_high_risk_threshold is not None) & (un_supervise_low_risk_threshold is not None):
                ...
                self.un_supervise_df = CRSValidationRiskLabeling \
        ...
        if static_rule_num_record > 0:
            ...
            if (static_rule_high_risk_threshold is not None) & (static_rule_low_risk_threshold is not None):
                ...
                self.static_rule_df = CRSValidationRiskLabeling \
        ...
        if dynamic_rule_num_record > 0:
            ...
            if (dynamic_rule_high_risk_threshold is not None) & (dynamic_rule_low_risk_threshold is not None):
                ...
                self.dynamic_rule_df = CRSValidationRiskLabeling \
        ...
        try:
            print("The thresholds are dynamic_rule_high_risk_threshold %s,dynamic_rule_low_risk_threshold %s, "
                  "static_rule_high_risk_threshold %s, static_rule_low_risk_threshold %s" % (
