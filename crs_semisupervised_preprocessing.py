...
try:
    from crs_semisupervised_configurations import ConfigCRSPreprocess, UNSUPERVISED_DATE_COL, TRAIN_UNSUPERVISED_MODE
    ...
    from CustomerRiskScoring.config.crs_semisupervised_configurations import ConfigCRSPreprocess, UNSUPERVISED_DATE_COL, \
        TRAIN_UNSUPERVISED_MODE
    ...
    from CustomerRiskScoring.config.crs_supervised_configurations import SUPERVISED_MAX_HISTORY

CDDALERTS.account_key = TMALERTS.account_key


class CrsSemiSupervisedPreprocess():
    """
    Preprocess:
    ...
    def __init__(self, txn_df=None, alerts_history_df=None, account_df=None, party_df=None, high_risk_party_df=None,
                 high_risk_country_df=None, c2a_df=None, alert_df=None, anomaly_history_df=None, broadcast_action=True,
                 target_values=None):
    ...
    def attach_max_date(self, cust_df, txn_df, alert_hist_df):
        ...
        :return:
        """
        txn_agg_expr = F.max(F.col(TRANSACTIONS.txn_date_time)).alias("TXN_" + UNSUPERVISED_DATE_COL)
        alert_agg_expr = F.max(F.col(CDDALERTS.alert_created_date)).alias("ALERT_" + UNSUPERVISED_DATE_COL)
        ...
        cust_ref_dt_df = cust_df.select(CUSTOMERS.party_key). \
            ...
            withColumn(UNSUPERVISED_DATE_COL,
                       F.coalesce(F.greatest(F.col("TXN_" + UNSUPERVISED_DATE_COL),
                                             F.col("ALERT_" + UNSUPERVISED_DATE_COL)),
                                  F.lit(current_date).cast(Defaults.TYPE_TIMESTAMP)))
        return cust_ref_dt_df.select(CUSTOMERS.party_key, UNSUPERVISED_DATE_COL)
    ...
    def select_table_cols(self):
        ...
        if self.anomaly_history_df != None:
            # this is optional and hence NO NEED TO RAISE ERROR if not provided
            # if it is available, select the required columns
            anomaly_cols = table_checker(min_cols=self.conf.minimal_anomaly_cols,
                                         custom_cols=self.conf.config_anomaly_cols, df=self.anomaly_history_df,
                                         table_name='ANOMALY')
            self.anomaly_history_df = self.anomaly_history_df.select(*anomaly_cols)
    ...
    def merge_anomaly_alert_history(self):
        ...
        if (self.anomaly_history_df != None) and (len(self.anomaly_history_df.head(1)) != 0):
            ...
            if sorted(self.alerts_history_df.columns) == sorted(anomaly_history_df.columns):
                ...
                # persist the union of alerts history and anomaly history
                self_alerts_history_df_count = check_and_broadcast(df=self.alerts_history_df,
                                                                   broadcast_action=self.broadcast_action,
                                                                   df_name='self.alerts_history_df')
            else:
                print(
                    "Preprocess: ANOMALY HISTORY DATA NOT ADDED TO ALERT HISTORY due to columns mismatch between both")
    ...
    def transaction_features_aggregation(self, high_risk_country_list=None, high_risk_party_list=None):
        ...
        final_txn_exprs = list(set(self.txn_df.columns) - set(txn_remove_used_cols)) + txn_exprs +[TRANSACTIONS.opp_country_code, self.conf.txn_opp_party_key]
    ...
    def encode_alert_status(self):
        ...
        if self.supervised or self.associated_entity:
    ...
    def _current_date_mode_process(self):
        ...
        """
        current_date = str(datetime.datetime.today())
        txn_df = self.txn_df.withColumn(UNSUPERVISED_DATE_COL, F.lit(current_date).cast(Defaults.TYPE_TIMESTAMP))
        alerts_history_df = self.alerts_history_df.withColumn(UNSUPERVISED_DATE_COL,
                                                              F.lit(current_date).cast(Defaults.TYPE_TIMESTAMP))
        cust_ref_dt_df = self.party_df.select(CUSTOMERS.party_key).withColumn(UNSUPERVISED_DATE_COL,
    ...
    def _overall_max_txn_date_mode_process(self):
        ...
        txn_df = self.txn_df.withColumn(UNSUPERVISED_DATE_COL, F.lit(max_trans_date).cast(Defaults.TYPE_TIMESTAMP))
        alerts_history_df = self.alerts_history_df.withColumn(UNSUPERVISED_DATE_COL,
    ...
    def run_unsupervised(self, train_predict_mode=None, prediction_reference_date_mode=None):
        """
        ...
        if train_predict_mode == TRAIN_UNSUPERVISED_MODE:
            cust_ref_dt_df, txn_df, alerts_history_df = self._individual_max_date_mode_process()
        else:
            print("The reference date mode is", prediction_reference_date_mode)
            if prediction_reference_date_mode == "INDIVIDUAL_MAX_DATE":
                cust_ref_dt_df, txn_df, alerts_history_df = self._individual_max_date_mode_process()
            elif prediction_reference_date_mode == "CURRENT_DATE":
                cust_ref_dt_df, txn_df, alerts_history_df = self._current_date_mode_process()
            elif prediction_reference_date_mode == "OVERALL_MAX_TXN_DATE":
        ...
        # Filter alert history data uptil maximum of 1 year based on UNSUPERVISED_DATE_COL
        # transaction filter happens in connector
        MAX_LOOKBACK_EXPR = 'INTERVAL {} DAYS'.format(str(SUPERVISED_MAX_HISTORY))
        # add party age
        txn_df = self.add_opp_party_age(txn_df)

        alerts_history_df = alerts_history_df. \
            withColumn(Defaults.MIN_LOOKBACK_DATE, (F.col(UNSUPERVISED_DATE_COL) - F.expr(MAX_LOOKBACK_EXPR))). \
            filter((F.col(CDDALERTS.alert_created_date) <= F.col(UNSUPERVISED_DATE_COL)) &
    ...
    def add_opp_party_age(self, alert_join_txn_df):
        ...
        if self.check_table_column(self.conf.txn_opp_date_of_birth, alert_join_txn_df):
            alert_join_txn_df = alert_join_txn_df.withColumn(self.conf.txn_opp_party_age,
                                                             (F.datediff(F.to_date(F.col(UNSUPERVISED_DATE_COL)),
                                                                         F.to_date(
                                                                             F.col(
...
#     pre = CrsSemiSupervisedPreprocess(TestData.transactions, TestData.alert_history, TestData.accounts, TestData.customers,
#                      TestData.high_risk_party, TestData.high_risk_country, TestData.c2a, None,
#                      TestData.anomaly_history, broadcast_action=False)
#     unsup_feature_map, ref_df, unsup_txn_df, unsup_alert_df = pre.run_unsupervised()
#     unsup_txn_df.printSchema()
