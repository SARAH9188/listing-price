...
try:
    from crs_prepipeline_tables import TRANSACTIONS, CDDALERTS, TMALERTS, ACCOUNTS, CUSTOMERS, C2A, C2C, HIGHRISKPARTY, HIGHRISKCOUNTRY
    from crs_postpipeline_tables import UI_CDDALERTS
    from crs_intermediate_tables import CDD_SUPERVISED_OUTPUT
    from crs_utils import table_checker
    from crs_semisupervised_configurations import ANOMALY_SCORING_COLS
    ...
    from CustomerRiskScoring.tables.crs_intermediate_tables import CDD_SUPERVISED_OUTPUT
    from CustomerRiskScoring.src.crs_utils.crs_utils import table_checker
    from CustomerRiskScoring.config.crs_semisupervised_configurations import ANOMALY_SCORING_COLS
    from Common.src.constants import Defaults
CDD_SUPERVISED_OUTPUT.account_key = TMALERTS.account_key

class ConfigCRSPreprocessAccount:
    def __init__(self):
        """
        ConfigCRSPreprocess:
        Currently for CRS there is no need for account preprocess as the account key won't be there from alerts side
        All configurable variables and values for CRS supervised preprocessing should be declared here
        """

        # Minimal columns that are required for the module to run, without which it errors
        # TRANSACTIONS
        self.minimal_txn_cols = [TRANSACTIONS.account_key,
                                 TRANSACTIONS.txn_date_time,
                                 TRANSACTIONS.txn_type_category,
                                 TRANSACTIONS.txn_amount,
                                 ]

        # TM_ALERTS orr CDD_ALERTS
        ...
        # HIGHRISKPARTY
        self.minimal_high_risk_party_cols = []

        # HIGHRISKCOUNTRY
        self.minimal_high_risk_country_cols = []
        ...
        # ACCOUNTS
        self.config_account_cols = self.minimal_account_cols + []

        # CUSTOMERS
        self.config_party_cols = self.minimal_party_cols + []
...
class ConfigCRSPreprocessCustomer:
    def __init__(self):
        """
        ConfigTMPreprocess:
        All configurable variables and values for CRS supervised preprocessing should be declared here
        ...
        # configurable columns: below column list will be used to select the required data from the actual tables
        # TRANSACTIONS
        self.config_txn_cols = self.minimal_txn_cols + [TRANSACTIONS.country_code,
        ...
        # ACCOUNTS
        self.config_account_cols = self.minimal_account_cols + [ACCOUNTS.segment_code,
                                                                ACCOUNTS.type_code,
        ...
        # CUSTOMERS
        self.config_party_cols = self.minimal_party_cols + [CUSTOMERS.individual_corporate_type,
                                                            CUSTOMERS.pep_flag,
                                                            CUSTOMERS.ngo_flag,
...
class ConfigCRSSupervisedFeatureBox:
    def __init__(self, feature_map=None):
        ...
        if feature_map is not None:
            ...
            self.opp_acc_key = self.feature_map.get(transaction_table).get('opp_acc_key')
            self.opp_acc_number = self.feature_map.get(transaction_table).get('opp_acc_number')
            self.opp_acc_key_num = self.feature_map.get(transaction_table).get('opp_acc_key_num')
        ...
        # lookback days for compare features
        compare_include_days = [90, 180, 360]
        self.compare_features_lookback_filter = {key: value for key, value in self.txn_lookback_filter.items()
                                                 if value in compare_include_days}
        ...
        self.txn_category_filter = {
            ...
            "incoming-all": ["CDM-cash-deposit", "cash-equivalent-deposit",
                             ...
                             "card-payment", "incoming-local-fund-transfer", "incoming-overseas-fund-transfer",
        ...
        self.operation_filter_wo_avg = {"AMT": F.sum, "VOL": F.count, "MAX": F.max, "SD": F.stddev}
        # operation - sum  only
        ...
        # operation - sum, count
        self.operation_sum_count_filter = {"AMT": F.sum, "VOL": F.count}
        # operation - sum, count, max and with stddev
        self.operation_wo_avg_sd_filter = {"AMT": F.sum, "VOL": F.count, "MAX": F.max}
        ...
        # in and out txn category mapping for avg operation
        self.avg_txn_mapping = {'AMT': ['VOL']}

        # first level operations that needed to be considered for avg operation eg: sum
        ...
        self.list_of_operations = list(self.operation_filter) + self.two_level_operation + \
                                  list(self.operation_count_filter)
...
MIN_LOOKBACK_DATE = 'MIN_LOOKBACK_' + CDDALERTS.alert_created_date
...
STATIC_ACCOUNT_FEATURES = ConfigStaticFeatures().account_cols + ['ACCOUNT_AGE']
...
STATIC_ALERT_FEATURES = ConfigStaticFeatures().alert_cols

SUPERVISED_MODEL_COLS = [
    CDD_SUPERVISED_OUTPUT.prediction_score,
    CDD_SUPERVISED_OUTPUT.prediction_bucket,
    '_explanation_xgb'
]
...
RI_COLS = [
    ...
    "ATM-withdrawal_30DAY_VOL_CUSTOMER",
    ...
    "card-payment_90DAY_AVG_CUSTOMER",
    "card-payment_180DAY_AVG_CUSTOMER",
...
ANOMALY_SCORING_COLS = ANOMALY_SCORING_COLS

UI_COLS = [
              CDD_SUPERVISED_OUTPUT.alert_id,
              CDD_SUPERVISED_OUTPUT.party_key,
              CDD_SUPERVISED_OUTPUT.account_key,
              CDD_SUPERVISED_OUTPUT.alert_created_date,
              CDD_SUPERVISED_OUTPUT.party_age
          ] + ConfigCRSSupervisedFeatureBox().customer_activity_features

EXP_BREAKDOWN_FIRST_LVL_COLS = ConfigCRSSupervisedFeatureBox().breakdown_first_level_features

DEDUP_MODULE_COLUMNS = list(set(BASE_COLS + SUPERVISED_MODEL_COLS + UNSUPERVISED_MODEL_1_COLS +
                                UNSUPERVISED_MODEL_2_COLS + ANOMALY_SCORING_COLS + UI_COLS))

UNION_BRANCH_CLUSTER_COLUMNS = list(set([
                                            CDD_SUPERVISED_OUTPUT.prediction_score,
                                            CDD_SUPERVISED_OUTPUT.prediction_bucket,
                                            CDD_SUPERVISED_OUTPUT.cluster_id,
                                            CDD_SUPERVISED_OUTPUT.party_age
                                        ] + BASE_COLS + UI_COLS))
...
class ErrorCodes:
    # error codes
    CDD_ERR_A100 = "CDD_ERR_A100"
    CDD_ERR_C100 = "CDD_ERR_C100"
    CDD_ERR_T100 = "CDD_ERR_T100"
    CDD_ERR_U100 = "CDD_ERR_U100"
