...
class ClusterMetaData(object):
    ...
    def __init__(self, table_name, tdss_dyn_prop=None):
        ...
        self.alert_col2 = self.MetaConfig.alertCol2
        self.str_col = self.MetaConfig.strCol
        self.str_col2 = self.MetaConfig.strCol2
        ...
        if self.table_name == 'CLUSTER_RISK_INDICATOR_STATS':
            self.selected_cols = ['PARTY_KEY'] + self.num_features + self.cat_features + [self.str_col, self.cluster_id,
                                                                                          self.alert_col, self.alert_col2, self.str_col2]
            ...
            self.selected_cols = list(set(['PARTY_KEY'] + self.num_features + self.cat_features +
                                          ...
                                          [self.str_col, self.cluster_id, self.alert_col,
                                           self.MetaConfig.cluster_score, self.alert_col2, self.str_col2]))
    ...
    def numerical_feature_range_stats(self, df, spark):
        ...
        expr = [(F.count(when((col(self.alert_col) > 0), True)) + F.count(when((col(self.alert_col2) > 0), True))).alias('HISTORICAL_ALERT_COUNT')]
        df_alertCount = df.groupBy(self.cluster_id).agg(*expr)
        expr = [(F.count(when((col(self.str_col) > 0), True)) + F.count(when((col(self.str_col2) > 0), True))).alias('HISTORICAL_STR_COUNT')]
    ...
    def run(self, spark, df, model_id):
        ...
        if self.table_name == self.CLUSTER_RISK_INDICATOR_STATS_tablename:
            ...
            if self.num_features == []:
                ...
                exprs = [F.sum(col(x)).alias(x + '_total') for x in list(set(self.MetaConfig.show_sum_cols))]
                ...
                exprs = [F.avg(col(x)).alias(x + '_avg') for x in list(set(self.MetaConfig.show_avg_cols))]
                ...
                exprs = [F.count(when(col(x) > 0, True)).alias(x + '_non_zero')
                         for x in list(set(self.MetaConfig.show_nonzero_cols))]
